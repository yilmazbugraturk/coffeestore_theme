	...
	private function print_customizer_upsell() {
		...
		if ( ! isset( $plugins['elementor/elementor.php'] ) ) {
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Install Elementor', 'coffee-store' ),
				__( 'Create a cross-site Header and Footer using Elementor & Hello theme', 'coffee-store' ),
				wp_nonce_url(
					add_query_arg(
						[
				...
				__( 'Install &amp; Activate', 'coffee-store' ),
				get_template_directory_uri() . '/assets/images/go-pro.svg'
			);
		} elseif ( ! defined( 'ELEMENTOR_VERSION' ) ) {
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Activate Elementor', 'coffee-store' ),
				__( 'Create a cross-site Header and Footer using Elementor & Hello theme', 'coffee-store' ),
			...
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Update Elementor', 'coffee-store' ),
				__( 'You need Elementor version 3.1.0 or above to create a cross-site Header and Footer.', 'coffee-store' ),
			...
			$customizer_content .= $this->get_customizer_upsell_html(
				__( 'Set Your Header &amp; Footer', 'coffee-store' ),
				__( 'Create cross-site Header and Footer using Elementor & Hello theme', 'coffee-store' ),
				wp_nonce_url( 'post.php?post=' . get_option( 'elementor_active_kit' ) . '&action=elementor' ),
				__( 'Start Here', 'coffee-store' ),
				get_template_directory_uri() . '/assets/images/go-pro.svg'
			);
		}
